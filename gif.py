...
class GifCog(commands.Cog):
    ...
    # ---------- TEXT COMMANDS ----------
    @commands.command()
    async def addgif(self, ctx, name: str, link: str):
        try:
    ...
    @commands.command()
    async def editgif(self, ctx, old_name: str, new_name: str = None, new_link: str = None):
        update = {}
        if new_name:
    ...
    @commands.command()
    async def delgif(self, ctx, name: str):
    ...
    @commands.command()
    async def listgifs(self, ctx, *, query: str = None):
        PER_PAGE = 10
    ...
    # ---------- SLASH COMMANDS ----------
    @app_commands.command(name="gifstop", description="Stop GIF responses")
    async def gifstop(self, interaction: discord.Interaction):
    ...
    @app_commands.command(name="gifstart", description="Start GIF responses")
    async def gifstart(self, interaction: discord.Interaction):
        settings_col.update_one(
            {"guild_id": interaction.guild.id},
            {"$set": {"enabled": True}},
            upsert=True
    ...
    @app_commands.command(name="giftime", description="Set GIF auto-delete time")
    @app_commands.describe(seconds="Time in seconds (1–300)")
    async def giftime(self, interaction: discord.Interaction, seconds: int):
